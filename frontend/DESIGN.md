...
### 카테고리 칩 팔레트 (학식 등 다중 카테고리 표시용, Phase B 추가)
브랜드 accent(teal)는 "손전등" 규율상 카테고리 구분에 쓰지 않음. 기존 하드코딩 hex를 값 변경 없이 CSS 변수로 승격.

| 토큰 | Light bg / fg | Dark bg / fg |
|---|---|---|
| `--chip-green` | `#eef5f0` / `#3a7a52` | `#1a2820` / `#5fb085` |
| `--chip-blue` | `#e6efff` / `#2e5fb3` | `#1a2030` / `#7aa5e3` |
| `--chip-red` | `#f5e6e2` / `#8a3a2c` | `#2a1818` / `#e07a6a` |
| `--chip-purple` | `#f0e8fc` / `#5b3aa8` | `#1f1828` / `#a78ce0` |
| `--chip-yellow` | `#fef6e6` / `#a07517` | `#2a2410` / `#d4a14a` |
| `--chip-gray` | `#f3f4f6` / `#6b7280` | `#1f2937` / `#9ca3af` |
...
## 8. 컴포넌트 규격 (2026-08 리디자인 — Before 점검 결과 반영)

2026-08-01 전 화면 실측 점검(모바일 390/PC 1440, computed-style 감사)에서 나온 결함 37건을
해소하며 확정한 규격. 상세 근거는 점검 보고서(아티팩트 before-audit) 참조.

### 8.1 TransitCard — 도착 카드 단일 규격 (`components/ui/TransitCard.jsx`)
버스·지하철·셔틀의 모든 도착 카드는 이 컴포넌트 하나만 쓴다. 해부도 고정:
`[노선배지][본문: 행선지 15px bold + 칩 행][ETA 열: 22px tnum + 보조 12.5px]`
- **제목 = 행선지**("시흥시청행"). 출발 정류장은 리스트 헤더에서 한 번만. **행선지 말줄임 금지, 최대 2줄** — 좁아지면 칩이 먼저 숨는다.
- 칩 순서 고정: 실시간 → 혼잡 → 경유. 카드마다 배치가 달라지는 것 금지.
- **임박(≤5분) = `--tj-imminent` 숫자색 하나만.** 카드 보더 강조·"곧 도착" 텍스트 대체·teal/초록 임박 표기 전부 금지.
- 미운행·정보 없음 = mute 계열(초록 금지)이며 반드시 대안 정보를 말한다("주말 미운행 · 월 첫차 06:30", "시간표 기준 15:20 출발").

### 8.2 SegmentedControl — 토글 단일 규격 (`components/ui/SegmentedControl.jsx`)
등교/하교 등 상호배타 토글은 전부 이것. 선택 = `--tj-pill-active-*`(라이트 잉크 필, 다크는 잉크 반전 — **teal 선택 배경 금지**). 정류장 필터 칩(민트 틴트 선택)과 시각 언어·줄을 분리해 같은 행에 두 그룹을 두지 않는다.

### 8.3 즐겨찾기 favKey 스키마 (`utils/favKey.js`)
`{mode}:{id}:{direction}` (예: `bus:3:하교`, `subway:suin:up`, `shuttle:main:등교`).
저장(`favorites.keys`)·필터·표시가 전부 이 키 하나를 읽는다. 레거시 값은 `matchesLegacy`로만 허용.

### 8.4 empty state 철칙
"왜 없는지 + 지금 뭘 할 수 있는지"를 반드시 말한다. 침묵 대시("—")·"추후 업데이트 예정"류 금지.
데이터가 있으면(학기 시간표 등) 반드시 이어준다. CTA는 문장이 아니라 버튼으로.

### 8.5 네거티브 룰 추가분
- **12px 미만 폰트 금지**는 `*.token.test`로 `src/**` 전역 강제(레이아웃 4개 파일 한정이던 검사 확장).
- **UI 렌더 문자열에 em-dash("—") 금지** — 구분은 "·", 침묵 대신 단어.
- 개발자 용어("표본 n건", "GBIS") UI 노출 금지 — 필요하면 툴팁으로.
- 히트맵·차트는 뷰포트 내 완결(가로 스크롤 금지) — 24칸이 안 들어가면 버킷을 줄인다(2시간×12칸).
- 위치 정보가 없으면 거리 표기를 숨긴다("42.0km" 같은 쓰레기 값 노출 금지).
