      ...
      <div className={`h-dvh bg-bg dark:bg-bg transition-colors duration-snap ease-inout ${isDesktop ? 'flex' : 'flex flex-col'}`}>
        ...
        <div className="flex flex-1 min-w-0 min-h-0 flex-col">
          ...
          {/* 모바일만 하단 FloatingDock. 데스크톱 네비는 PCSidebar가 담당한다.
              더보기 진입점은 예전엔 여기 position:fixed 오버레이 버튼이었다 —
              모바일 모든 화면 위에 항상 떠서, 히어로 상단 스트립의 뷰 토글
              아이콘과 같은 자리에 겹쳐 그 아이콘을 가리는 버그가 났다(사용자 실측).
              문서 흐름 밖(fixed)이라 생긴 문제라 오버레이 자체를 없앴다 — 새
              진입점은 HomeWeatherHero.jsx의 "히어로 옵션" 아이콘 그룹 안(문서
              흐름 안, 다른 요소를 덮지 않음)에 4번째 아이콘으로 옮겼다. 홈
              화면(지금 뷰)에서만 보이지만, 하단 독의 "홈" 탭이 모든 모바일
              화면에 항상 떠 있어 최대 두 번(홈 → 더보기)이면 어디서든 닿는다 —
              /favorites가 도달 경로 자체 없이 죽어 있던 것과는 다른 상황이다. */}
