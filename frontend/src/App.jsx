      ...
      <div className={`h-dvh bg-bg dark:bg-bg transition-colors duration-snap ease-inout ${isDesktop ? 'flex' : 'flex flex-col'}`}>
        ...
        <div className="flex flex-1 min-w-0 min-h-0 flex-col">
          ...
          {/* 모바일만 하단 FloatingDock. 데스크톱 네비는 PCSidebar가 담당한다.
              더보기 진입점은 한때 여기 position:fixed 오버레이 버튼이었다가(모바일
              모든 화면 위에 항상 떠서 히어로 상단 스트립의 뷰 토글 아이콘을 가리는
              버그가 났다, 사용자 실측), 그 다음엔 HomeWeatherHero.jsx의 "히어로
              옵션" 아이콘 그룹으로 옮겨 홈 화면에서만 노출됐다. 이제는 독 자체가
              5탭(홈/시간표/학교시설/공지/더보기)이라 더보기가 독의 정식 탭으로
              돌아왔다 — 학식·매장 두 탭이 "학교시설" 한 탭으로 합쳐지며 생긴 빈
              칸을 채웠다(FloatingDock.jsx 참고). 어느 모바일 화면에서든 한 번만
              누르면 닿는다 — /favorites가 도달 경로 자체 없이 죽어 있던 것과는
              다른 상황이다. */}
