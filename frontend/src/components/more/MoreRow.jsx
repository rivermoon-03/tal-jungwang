...
export default function MoreRow({
  ...
  const borderClass = !last ? 'border-b border-slate-100 dark:border-border-dark' : ''
  ...
  const inner = (
    <div className="flex items-center gap-3 px-4 py-3.5">
      ...
      {badge != null && badge > 0 && (
        <span className="inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 rounded-full bg-accent dark:bg-accent-dark text-white dark:text-ink text-micro font-bold">
          {badge}
        </span>
      )}
      {value != null && (
  ...
  const cls = `block w-full text-left bg-white dark:bg-surface-dark transition-colors hover:bg-slate-50 dark:hover:bg-slate-700/50 active:bg-slate-100 ${borderClass} ${radiusClass} overflow-hidden`
