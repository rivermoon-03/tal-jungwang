/**
 * AcademicCalendarGrid — 주간/월간 그리드 캘린더 단위 테스트.
 * 날짜는 항상 고정된 `now`/이벤트를 넘겨 오늘 날짜에 의존하지 않게 한다.
 *
 * 결함 #34: 기본 뷰가 거대한 월간 그리드였던 것을 주간 스트립 1줄로 낮췄다
 * (탭: "이번 주"(기본)/"월 전체보기"). 월간 그리드 관련 테스트는 먼저
 * "월 전체보기" 탭으로 전환한 뒤 검증한다.
 */
...
describe('AcademicCalendarGrid — 그리드 렌더링(월 전체보기)', () => {
  it('요일 헤더 7개를 렌더링한다', () => {
    render(<AcademicCalendarGrid events={[]} now={NOW} />)
    switchToMonth()
    // '이번 주' 탭과 요일 헤더가 섞이지 않도록 헤더 영역으로 스코프한다.
...
describe('AcademicCalendarGrid — 이벤트 마커(월 전체보기)', () => {
...
describe('AcademicCalendarGrid — 월 이동(월 전체보기)', () => {
...
describe('AcademicCalendarGrid — 날짜 선택(월 전체보기)', () => {
...
describe('AcademicCalendarGrid — 주/월 탭', () => {
  it('기본은 주간 뷰(이번 주)이고, "이번 주"/"월 전체보기" 탭이 렌더링된다', () => {
    render(<AcademicCalendarGrid events={[]} now={NOW} />)
    expect(screen.getByRole('tab', { name: '이번 주' })).toHaveAttribute('aria-selected', 'true')
    expect(screen.getByRole('tab', { name: '월 전체보기' })).toHaveAttribute('aria-selected', 'false')
    // 오늘(2026-07-18, 토)이 속한 주(7/12~7/18)가 바로 보여야 한다.
