...
export default function SubwayLineCard({ lineName, dirLabel, color, darkColor, lightColor, trains, onClick }) {
  ...
  return (
    <div
      className="rounded-xl overflow-hidden border border-slate-200 dark:border-border-dark shadow-sm cursor-pointer pressable"
      onClick={onClick}
    >
      {/* 컬러 헤더 */}
      ...
      <div
        className="px-4 py-3 dark:bg-surface-dark"
      ...
      {/* 이후 열차 목록 */}
      {preview.length > 0 && (
        <ul className="divide-y divide-slate-100 dark:divide-slate-700 bg-white dark:bg-surface-dark">
          {preview.map((t, i) => (
