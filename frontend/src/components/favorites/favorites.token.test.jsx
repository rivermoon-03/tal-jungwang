/**
 * favorites/ 컴포넌트 AI티 제거 토큰 준수 테스트
 *
 * 검증 항목:
...
const FAVORITES_DIR = path.resolve(__dirname)

const TARGET_FILES = [
  'FavoritesList.jsx',
  'FavoritesTimeline.jsx',
  'FavoritesPage.jsx',
]

function readFile(name) {
  return fs.readFileSync(path.join(FAVORITES_DIR, name), 'utf8')
}

// ── 1. text-slate-* / text-gray-* 생색 금지 ───────────────────────────
describe('favorites/ 토큰 준수 — text-slate-*/text-gray-* 생색 금지', () => {
...
describe('favorites/ 토큰 준수 — bg-slate-*/bg-gray-* 생색 금지', () => {
  TARGET_FILES.forEach((file) => {
...
// ── 3. 12px 미만 인라인 폰트 금지 ─────────────────────────────────────
describe('favorites/ 토큰 준수 — 12px 미만 폰트 금지', () => {
  TARGET_FILES.forEach((file) => {
