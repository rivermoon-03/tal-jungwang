...
export default function WalkRouteCard() {
  ...
  return (
    <div className="absolute bottom-28 md:bottom-4 left-3 z-[65] pointer-events-auto
                    bg-surface dark:bg-surface-dark rounded-2xl shadow-2xl
      ...
      <div className="flex-1 min-w-0">
        <p className="text-body font-bold text-ink dark:text-ink-dark truncate">
          {walkRoute.destName ?? '목적지'}까지 도보
        </p>
        <p className="text-caption text-mute dark:text-mute-dark tabular-nums">
      ...
      <button
        ...
        className="w-7 h-7 rounded-full bg-surface-2 dark:bg-surface-2-dark flex items-center justify-center
                   text-mute dark:text-mute-dark hover:text-ink dark:hover:text-ink-dark
                   hover:bg-line dark:hover:bg-line-dark transition-colors flex-shrink-0"
      >
        <X size={14} />
      </button>
    </div>
