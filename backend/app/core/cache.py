...
async def get_or_fetch_with_lock(
    ...
    *,
    ...
    - `ttl`은 고정 int 또는 `fetch_fn` 결과를 받아 int를 반환하는 콜러블이다.
      결과값에 따라 캐시 수명을 달리하고 싶을 때(예: positive/negative caching) 사용한다.
    ...
    if acquired:
        try:
            ...
            resolved_ttl = ttl(result) if callable(ttl) else ttl
            await set_cached_json(key, result, resolved_ttl)
            return result
        finally:
            try:
