...
`android/widget/` 에 홈 화면 위젯 소스가 준비돼 있다 — 앱을 열지 않고도 다음
셔틀·버스·지하철을 홈에서 본다. TWA 프로젝트에 파일만 복사하면 된다.

- 데이터: `GET /api/v1/widget` (위젯 전용 경량 엔드포인트 — 표기 문자열까지 서버가 완성)
- 색: 앱 semantic 토큰 그대로(라이트/다크 자동)
- 갱신: 30분 주기 + 잠금 해제 시 + ↻ 탭

자세한 절차·디자인 스펙은 `android/widget/README.md` 참고.
